...
do
    ...
    if [[ "$flag" == "1" ]]; then
        ...
        udir=$(mktemp -d)
        runScript $quicExeFile $vid " " $timing "quic" "Modified"
        chmod +x $quicExeFile

        sudo bash tcpdump_start.sh pcaps/${vid}_"${log_type}"_quic_${timing}_Modified
        ...
        cp ${udir}/chrome_debug.log logs/chrome_debug_${vid}_"${log_type}"_quic_${timing}_Modified.log
        cp logs/chrome_debug_${vid}_"${log_type}"_quic_${timing}_Modified.log logs-collected/chrome_debug_${vid}_"${log_type}"_quic_${timing}_Modified.log
    ...
    if [[ "$flag" == "1" ]]; then
        ...
        sudo bash tcpdump_start.sh pcaps/${vid}_"${log_type}"_quic_${timing}_Original
        ...
        cp ${udir}/chrome_debug.log logs/chrome_debug_${vid}_"${log_type}"_quic_${timing}_Original.log
        cp logs/chrome_debug_${vid}_"${log_type}"_quic_${timing}_Original.log logs-collected/chrome_debug_${vid}_"${log_type}"_quic_${timing}_Original.log
