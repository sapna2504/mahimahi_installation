...
do
    ...
    if [[ "$flag" == "1" ]]; then
        ...
        udir=$(mktemp -d)
        runScript $quicExeFile $vid " " $timing "quic" "Alpha"
        chmod +x $quicExeFile

        sudo bash tcpdump_start.sh pcaps/${vid}_"${log_type}"_quic_${timing}_${FITZ_THRESHOLD}
        ...
        cp ${udir}/chrome_debug.log logs/chrome_debug_${vid}_"${log_type}"_quic_${timing}_${FITZ_THRESHOLD}.log
        cp logs/chrome_debug_${vid}_"${log_type}"_quic_${timing}_${FITZ_THRESHOLD}.log logs-collected/chrome_debug_${vid}_"${log_type}"_quic_${timing}_${FITZ_THRESHOLD}.log
  
        rm -rf $udir
        rm $quicExeFile
        echo $vid + "quic modified complete"
        sleep 10
    fi
done
