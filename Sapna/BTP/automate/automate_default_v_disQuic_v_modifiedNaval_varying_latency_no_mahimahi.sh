...
exeFile=$(mktemp)
ifconfig_result=$(ifconfig|grep 'enp\|eth0')
INTERFACE_NAME=(${ifconfig_result//:/ }[0])

sudo tc qdisc add dev $INTERFACE_NAME root netem delay 1000ms 200ms distribution normal
...
sudo tc qdisc del dev $INTERFACE_NAME root netem delay 1000ms 200ms distribution normal
