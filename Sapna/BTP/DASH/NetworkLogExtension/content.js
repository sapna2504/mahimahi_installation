	...
	if (msg.indexOf("streamingstats") > 0) {
		if (msg.indexOf("COMPLETE") > -1) {
			...
			if (bh_index > 0) {
				...
				// console.log("AAAAA", bh_values, buffer)
				// if (get_video_duration() !== null && buffer >= get_video_duration()) {
				// 	console.log("Closing window", buffer, get_video_duration())
				// 	set_video_duration(null)
				// 	window.close();
				// }
			}
		}
